...
public class IngredientCommandServiceImpl implements IngredientCommandService {
  ...
  public Optional<Ingredient> handle(UpdateIngredientCommand command) {
    IngredientCode code = command.code();
    Optional<Ingredient> existing = ingredientRepository.findByCode(code);
    ...
    ing.setDefaultPortionGrams(command.defaultPortionGrams());
